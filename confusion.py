...
class ConfusionMatrix:
    ...
    def __init__(self, classes=None):
        ...
        self.switcher_classifier = 0
        self.last_true_label = None
        self.past2_label = None
    ...
    def update(self, y_true, y_pred, sample_weight=1.0):
        ...
        if self.get_majority_class() == y_true:
            self.majority_classifier += sample_weight
        if self.last_true_label == y_true:
            self.correct_no_change += sample_weight
        if self.past2_label == y_true:
            self.switcher_classifier += sample_weight
        if self.n_samples > 2:
            self.past2_label = self.last_true_label
    ...
    def get_majority_class(self):
        return max(self.sum_row, key=self.sum_row.get)
